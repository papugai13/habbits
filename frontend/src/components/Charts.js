...
const HabitsComparisonChart = ({ period, viewType, currentWeekDate, selectedCategory }) => {
    ...
    useEffect(() => {
        const fetchComparisonData = async () => {
            setLoading(true);
            try {
                const response = await fetch(`/api/v1/habits/habit_comparison/?period=${period}&date=${currentWeekDate}&category=${encodeURIComponent(selectedCategory || 'Все')}`, {
                    credentials: 'include'
                });
                if (response.ok) {
...
const Charts = ({ 
    ...
    const fetchStatistics = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/v1/habits/daily_statistics/?period=${period}&date=${chartDate}&category=${encodeURIComponent(selectedCategory || 'Все')}`, {
                credentials: 'include'
            });
